...
struct TeachingLoad
{
    // Ставка за зачёты/экзамены фиксирована и не меняется в UI,
    // поэтому хранится как константа метода, а не как поле БД/структуры.
...
// Входные данные для расчёта зарплаты — заполняются вызывающим кодом
// и передаются в calculateSalary() одним объектом вместо списка параметров.
struct SalaryInput
{
    int employeeId = -1;
    QString period;
